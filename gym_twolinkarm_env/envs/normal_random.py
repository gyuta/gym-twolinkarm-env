...
import gym_twolinkarm_env.envs.func as func


class NormalRandom(gym.Env):
    ...
    def __init__(self):
        ...
        o_high = np.array([np.pi, np.pi, np.finfo(np.float32).max, np.finfo(np.float32).max, 1.0, 1.0], dtype=np.float32)
    ...
    def step(self, u):
        ...
        obs = np.concatenate((self.state, self.target))
